export default function MapsReducer(state={
  ...
  switch(action.type){
    ...
    case "FILTER_MAP":
      if (action.payload ===  ""){
        return {...state, selectedMap: state.maps[0] || {markers: []}}
      }else{
    ...
    case "REMOVE_MARKER_FROM_MAP":
      return {...state, selectedMap: {...state.selectedMap, markers: state.selectedMap.markers.filter(m=>m.id !== action.payload.id)}}
    default:
      return state
  }
}